#ifndef SXS_BLOCK_POLICY_BPF_CORE_H
#define SXS_BLOCK_POLICY_BPF_CORE_H

#include "policy-bpf-abi.h"

#define BLOCK_MAP_OBJECT 1U
#define BLOCK_REVERSE_MAP_OBJECT 2U
#define BLOCK_METADATA_OBJECT 3U
#define BLOCK_ESWD_CLASS_OBJECT 4U

#define BLOCK_PAIR_READ 1U
#define BLOCK_PAIR_WRITE 2U
#define BLOCK_PAIR_DSM 3U
#define BLOCK_PAIR_BACKGROUND_GC 4U

#define BLOCK_NVME_READ 0x02U
#define BLOCK_NVME_WRITE 0x01U
#define BLOCK_NVME_DSM 0x09U
#define BLOCK_NVME_SUCCESS 0x0000U
#define BLOCK_NVME_INVALID_FIELD 0x4002U
#define BLOCK_NVME_INTERNAL_ERROR 0x4006U
#define BLOCK_MAX_DSM_PAGES 65536ULL

#define BLOCK_UNMAPPED (~0ULL)

enum block_eswd_class {
    BLOCK_ESWD_FREE = 0,
    BLOCK_ESWD_CURRENT = 1,
    BLOCK_ESWD_FULL = 2,
    BLOCK_ESWD_VICTIM = 3,
    BLOCK_ESWD_RECLAIMING = 4,
};

struct block_bpf_metadata {
    sxs_u64 total_logical_pages;
    sxs_u32 total_eswds;
    sxs_u32 pages_per_eswd;
    sxs_u32 sectors_per_page;
    sxs_u32 sector_size;
    sxs_u32 page_size;
    sxs_u32 current_eswd;
    sxs_u32 free_eswds;
    sxs_u32 gc_low_watermark;
    sxs_u32 gc_urgent_watermark;
};

enum block_scratch_offset {
    BLOCK_PAGE_OFFSET = 0,
    BLOCK_PAGE_REQUEST_OFFSET = 4096,
    BLOCK_PAGE_RESULT_OFFSET = 4160,
    BLOCK_GEOMETRY_OFFSET = 4224,
    BLOCK_METADATA_OFFSET = 4320,
    BLOCK_ESWD_OFFSET = 4384,
    BLOCK_DSM_OFFSET = 4416,
    BLOCK_TEMP_OFFSET = 4448,
    BLOCK_CLASS_OFFSET = 4480,
};

/* Compile-time extensions preserve direct condition/action control flow. */
#ifndef BLOCK_EXTRA_INIT
#define BLOCK_EXTRA_INIT(context, geometry, metadata) 0
#endif
#ifndef BLOCK_EXTRA_CONDITION
#define BLOCK_EXTRA_CONDITION(context) 0
#endif
#ifndef BLOCK_EXTRA_ACTION
#define BLOCK_EXTRA_ACTION(context) SXS_BPF_ACTION_ERROR
#endif
#ifndef BLOCK_PREPARE_APPEND
#define BLOCK_PREPARE_APPEND(context, lpn, old_ppa, request) 0
#endif
#ifndef BLOCK_HANDLE_OLD_PAGE
#define BLOCK_HANDLE_OLD_PAGE(context, lpn, old_ppa) \
    sxs_page_invalidate((old_ppa), 0, 0, 0, 0)
#endif
#ifndef BLOCK_AFTER_NEW_PAGE
#define BLOCK_AFTER_NEW_PAGE(context, lpn, old_ppa, new_ppa) 0
#endif
#ifndef BLOCK_ON_PAGE_READ
#define BLOCK_ON_PAGE_READ(context, lpn, ppa) 0
#endif
#ifndef BLOCK_GC_SHOULD_MIGRATE
#define BLOCK_GC_SHOULD_MIGRATE(context, ppa) 1
#endif
#ifndef BLOCK_GC_AFTER_MIGRATE
#define BLOCK_GC_AFTER_MIGRATE(context, old_ppa, new_ppa, lpn) 0
#endif
#ifndef BLOCK_GC_BEFORE_ERASE
#define BLOCK_GC_BEFORE_ERASE(context, victim, metadata) 0
#endif

static void block_bytes_zero(sxs_u8 *destination, sxs_u32 length)
{
    for (sxs_u32 i = 0; i < length; i++) {
        destination[i] = 0;
    }
}

static sxs_s64 block_state_read_u64(struct sxs_bpf_context *context,
                                    sxs_u32 object_id, sxs_u64 index,
                                    sxs_u64 *value_out)
{
    sxs_s64 rc = sxs_state_read(object_id, index, 0,
                                BLOCK_TEMP_OFFSET, sizeof(sxs_u64));

    if (rc == 0) {
        *value_out = *(sxs_u64 *)(context->scratch + BLOCK_TEMP_OFFSET);
    }
    return rc;
}

static sxs_s64 block_state_write_u64(struct sxs_bpf_context *context,
                                     sxs_u32 object_id, sxs_u64 index,
                                     sxs_u64 value)
{
    *(sxs_u64 *)(context->scratch + BLOCK_TEMP_OFFSET) = value;
    return sxs_state_write(object_id, index, 0,
                           BLOCK_TEMP_OFFSET, sizeof(value));
}

static sxs_s64 block_state_read_class(struct sxs_bpf_context *context,
                                      sxs_u32 eswd_id, sxs_u8 *class_out)
{
    sxs_s64 rc = sxs_state_read(BLOCK_ESWD_CLASS_OBJECT, eswd_id, 0,
                                BLOCK_CLASS_OFFSET, 1);

    if (rc == 0) {
        *class_out = context->scratch[BLOCK_CLASS_OFFSET];
    }
    return rc;
}

static sxs_s64 block_state_write_class(struct sxs_bpf_context *context,
                                       sxs_u32 eswd_id, sxs_u8 value)
{
    context->scratch[BLOCK_CLASS_OFFSET] = value;
    return sxs_state_write(BLOCK_ESWD_CLASS_OBJECT, eswd_id, 0,
                           BLOCK_CLASS_OFFSET, 1);
}

static sxs_s64 block_read_metadata(struct sxs_bpf_context *context,
                                   struct block_bpf_metadata **metadata_out)
{
    sxs_s64 rc = sxs_state_read(BLOCK_METADATA_OBJECT, 0, 0,
                                BLOCK_METADATA_OFFSET,
                                sizeof(struct block_bpf_metadata));

    if (rc == 0) {
        *metadata_out = (struct block_bpf_metadata *)(context->scratch +
                                                       BLOCK_METADATA_OFFSET);
    }
    return rc;
}

static sxs_s64 block_write_metadata(void)
{
    return sxs_state_write(BLOCK_METADATA_OBJECT, 0, 0,
                           BLOCK_METADATA_OFFSET,
                           sizeof(struct block_bpf_metadata));
}

static sxs_s64 block_get_eswd(struct sxs_bpf_context *context,
                              sxs_u32 eswd_id,
                              struct sxs_bpf_eswd **eswd_out)
{
    sxs_s64 rc = sxs_eswd_get(eswd_id, BLOCK_ESWD_OFFSET, 0, 0, 0);

    if (rc == 0) {
        *eswd_out = (struct sxs_bpf_eswd *)(context->scratch +
                                             BLOCK_ESWD_OFFSET);
    }
    return rc;
}

static sxs_s64 block_find_free_eswd(struct sxs_bpf_context *context,
                                    const struct block_bpf_metadata *metadata,
                                    sxs_u32 *eswd_out)
{
    for (sxs_u32 id = 0; id < metadata->total_eswds; id++) {
        sxs_u8 class_value;

        if (block_state_read_class(context, id, &class_value) != 0) {
            return -SXS_BPF_EIO;
        }
        if (class_value == BLOCK_ESWD_FREE) {
            *eswd_out = id;
            return 0;
        }
    }
    return -SXS_BPF_ENOSPC;
}

static sxs_s64 block_rotate_if_full(struct sxs_bpf_context *context,
                                    struct block_bpf_metadata *metadata)
{
    struct sxs_bpf_eswd *current;
    sxs_u32 next;

    if (block_get_eswd(context, metadata->current_eswd, &current) != 0) {
        return -SXS_BPF_EIO;
    }
    if (current->write_page_index < metadata->pages_per_eswd) {
        return 0;
    }
    if (block_state_write_class(
            context, metadata->current_eswd,
            current->invalid_page_count == 0 ? BLOCK_ESWD_FULL :
                                                BLOCK_ESWD_VICTIM) != 0 ||
        block_find_free_eswd(context, metadata, &next) != 0 ||
        block_state_write_class(context, next, BLOCK_ESWD_CURRENT) != 0) {
        return -SXS_BPF_ENOSPC;
    }
    metadata->current_eswd = next;
    if (metadata->free_eswds == 0) {
        return -SXS_BPF_ENOSPC;
    }
    metadata->free_eswds--;
    return block_write_metadata();
}

static sxs_s64 block_page_read(struct sxs_bpf_context *context, sxs_u64 ppa,
                               sxs_u32 page_size, sxs_u64 *latency_out)
{
    struct sxs_bpf_page_read_request *request =
        (struct sxs_bpf_page_read_request *)(context->scratch +
                                              BLOCK_PAGE_REQUEST_OFFSET);
    struct sxs_bpf_page_result *result =
        (struct sxs_bpf_page_result *)(context->scratch +
                                        BLOCK_PAGE_RESULT_OFFSET);
    sxs_s64 rc;

    request->ppa = ppa;
    request->page_offset = 0;
    request->length = page_size;
    request->data_offset = BLOCK_PAGE_OFFSET;
    request->result_offset = BLOCK_PAGE_RESULT_OFFSET;
    request->oob_object_id = 0;
    request->oob_offset = 0;
    request->oob_length = 0;
    rc = sxs_page_read(BLOCK_PAGE_REQUEST_OFFSET, 0, 0, 0, 0);
    if (rc == 0 && result->status == 0) {
        *latency_out = result->latency_ns;
        return 0;
    }
    return -SXS_BPF_EIO;
}

static sxs_s64 block_page_append(struct sxs_bpf_context *context,
                                 sxs_u64 lpn, sxs_u64 old_ppa,
                                 sxs_u32 eswd_id, sxs_u32 page_size,
                                 sxs_u64 *ppa_out, sxs_u64 *latency_out)
{
    struct sxs_bpf_page_append_request *request =
        (struct sxs_bpf_page_append_request *)(context->scratch +
                                                BLOCK_PAGE_REQUEST_OFFSET);
    struct sxs_bpf_page_result *result =
        (struct sxs_bpf_page_result *)(context->scratch +
                                        BLOCK_PAGE_RESULT_OFFSET);
    sxs_s64 rc;

    (void)lpn;
    (void)old_ppa;

    request->eswd_id = eswd_id;
    request->data_offset = BLOCK_PAGE_OFFSET;
    request->data_length = page_size;
    request->result_offset = BLOCK_PAGE_RESULT_OFFSET;
    request->oob_object_id = 0;
    request->oob_offset = 0;
    request->oob_length = 0;
    request->reserved = 0;
    if (BLOCK_PREPARE_APPEND(context, lpn, old_ppa, request) != 0) {
        return -SXS_BPF_EIO;
    }
    rc = sxs_page_append(BLOCK_PAGE_REQUEST_OFFSET, 0, 0, 0, 0);
    if (rc == 0 && result->status == 0) {
        *ppa_out = result->ppa;
        *latency_out = result->latency_ns;
        return 0;
    }
    return -SXS_BPF_EIO;
}

static sxs_s64 block_update_mapping(struct sxs_bpf_context *context,
                                    sxs_u64 lpn, sxs_u64 old_ppa,
                                    sxs_u64 new_ppa)
{
    sxs_u64 page_index;

    if (old_ppa != BLOCK_UNMAPPED) {
        page_index = sxs_ppa_to_page_index(old_ppa, 0, 0, 0, 0);
        if ((sxs_s64)page_index >= 0 &&
            block_state_write_u64(context, BLOCK_REVERSE_MAP_OBJECT,
                                  page_index, BLOCK_UNMAPPED) != 0) {
            return -SXS_BPF_EIO;
        }
    }
    if (block_state_write_u64(context, BLOCK_MAP_OBJECT, lpn, new_ppa) != 0) {
        return -SXS_BPF_EIO;
    }
    if (new_ppa != BLOCK_UNMAPPED) {
        page_index = sxs_ppa_to_page_index(new_ppa, 0, 0, 0, 0);
        if ((sxs_s64)page_index < 0 ||
            block_state_write_u64(context, BLOCK_REVERSE_MAP_OBJECT,
                                  page_index, lpn) != 0) {
            return -SXS_BPF_EIO;
        }
    }
    return 0;
}

static sxs_s64 block_select_victim(struct sxs_bpf_context *context,
                                   const struct block_bpf_metadata *metadata,
                                   sxs_u32 force, sxs_u32 *victim_out)
{
    sxs_u32 best = 0;
    sxs_s32 best_vpc = 0x7fffffff;
    sxs_u32 found = 0;

    for (sxs_u32 id = 0; id < metadata->total_eswds; id++) {
        struct sxs_bpf_eswd *eswd;
        sxs_u8 class_value;

        if (id == metadata->current_eswd ||
            block_state_read_class(context, id, &class_value) != 0) {
            continue;
        }
        if (class_value != BLOCK_ESWD_FULL &&
            class_value != BLOCK_ESWD_VICTIM) {
            continue;
        }
        if (block_get_eswd(context, id, &eswd) != 0 ||
            (!force && eswd->invalid_page_count <
                           (sxs_s32)(metadata->pages_per_eswd / 8))) {
            continue;
        }
        if (eswd->valid_page_count < best_vpc) {
            best_vpc = eswd->valid_page_count;
            best = id;
            found = 1;
        }
    }
    if (!found) {
        return -SXS_BPF_ENOENT;
    }
    *victim_out = best;
    return 0;
}

static sxs_s64 block_gc(struct sxs_bpf_context *context,
                        struct block_bpf_metadata *metadata, sxs_u32 force)
{
    struct sxs_bpf_page_migrate_request *request =
        (struct sxs_bpf_page_migrate_request *)(context->scratch +
                                                 BLOCK_PAGE_REQUEST_OFFSET);
    struct sxs_bpf_page_result *result =
        (struct sxs_bpf_page_result *)(context->scratch +
                                        BLOCK_PAGE_RESULT_OFFSET);
    sxs_u32 victim;
    sxs_u64 migrated = 0;
    sxs_u64 gc_start;

    if (block_select_victim(context, metadata, force, &victim) != 0 ||
        block_state_write_class(context, victim,
                                BLOCK_ESWD_RECLAIMING) != 0) {
        return -SXS_BPF_ENOENT;
    }
    gc_start = sxs_time_now_ns(0, 0, 0, 0, 0);
    sxs_stats_gc_active_set(1, 0, 0, 0, 0);
    for (sxs_u32 page_index = 0;
         page_index < metadata->pages_per_eswd; page_index++) {
        sxs_u64 source_ppa;
        sxs_u64 source_dense_index;
        sxs_u64 lpn = BLOCK_UNMAPPED;

        if (sxs_eswd_to_ppa(victim, page_index,
                            BLOCK_TEMP_OFFSET, 0, 0) != 0) {
            continue;
        }
        source_ppa = *(sxs_u64 *)(context->scratch + BLOCK_TEMP_OFFSET);
        if ((sxs_s64)sxs_page_status_get(source_ppa, 0, 0, 0, 0) != 2) {
            continue;
        }
        if (!BLOCK_GC_SHOULD_MIGRATE(context, source_ppa)) {
            continue;
        }
        if (block_rotate_if_full(context, metadata) != 0) {
            sxs_stats_gc_active_set(0, 0, 0, 0, 0);
            return -SXS_BPF_ENOSPC;
        }
        request->source_ppa = source_ppa;
        request->destination_eswd_id = metadata->current_eswd;
        request->result_offset = BLOCK_PAGE_RESULT_OFFSET;
        if (sxs_page_migrate(BLOCK_PAGE_REQUEST_OFFSET, 0, 0, 0, 0) != 0 ||
            result->status != 0) {
            sxs_stats_gc_active_set(0, 0, 0, 0, 0);
            return -SXS_BPF_EIO;
        }
        source_dense_index = sxs_ppa_to_page_index(source_ppa, 0, 0, 0, 0);
        if ((sxs_s64)source_dense_index >= 0 &&
            block_state_read_u64(context, BLOCK_REVERSE_MAP_OBJECT,
                                 source_dense_index, &lpn) == 0 &&
            lpn != BLOCK_UNMAPPED && lpn < metadata->total_logical_pages) {
            if (block_update_mapping(context, lpn, source_ppa,
                                     result->ppa) != 0) {
                sxs_stats_gc_active_set(0, 0, 0, 0, 0);
                return -SXS_BPF_EIO;
            }
        }
        if (BLOCK_GC_AFTER_MIGRATE(context, source_ppa, result->ppa,
                                   lpn) != 0) {
            sxs_stats_gc_active_set(0, 0, 0, 0, 0);
            return -SXS_BPF_EIO;
        }
        migrated++;
    }
    if (BLOCK_GC_BEFORE_ERASE(context, victim, metadata) != 0) {
        sxs_stats_gc_active_set(0, 0, 0, 0, 0);
        return -SXS_BPF_EIO;
    }
    sxs_eswd_erase(victim, 0, 0, 0, 0);
    if (sxs_eswd_reset(victim, 0, 0, 0, 0) != 0 ||
        block_state_write_class(context, victim, BLOCK_ESWD_FREE) != 0) {
        sxs_stats_gc_active_set(0, 0, 0, 0, 0);
        return -SXS_BPF_EIO;
    }
    metadata->free_eswds++;
    if (block_write_metadata() != 0) {
        sxs_stats_gc_active_set(0, 0, 0, 0, 0);
        return -SXS_BPF_EIO;
    }
    sxs_stats_add(SXS_BPF_STATS_GC_INVOCATIONS, 1, 0, 0, 0);
    sxs_stats_add(SXS_BPF_STATS_GC_PAGES_MIGRATED, migrated, 0, 0, 0);
    sxs_stats_add(force ? SXS_BPF_STATS_FOREGROUND_GC :
                          SXS_BPF_STATS_BACKGROUND_GC,
                  1, 0, 0, 0);
    sxs_stats_add(SXS_BPF_STATS_GC_TIME_NS,
                  sxs_time_now_ns(0, 0, 0, 0, 0) - gc_start,
                  0, 0, 0);
    sxs_stats_gc_active_set(0, 0, 0, 0, 0);
    return 0;
}

static sxs_u64 block_read_action(struct sxs_bpf_context *context,
                                 struct block_bpf_metadata *metadata)
{
    sxs_u64 maximum_latency = 0;
    sxs_u64 end_lba = context->event.nvme.lba +
                      context->event.nvme.nsecs;
    sxs_u64 current_lba = context->event.nvme.lba;

    if (end_lba < current_lba) {
        sxs_completion_status_set(BLOCK_NVME_INVALID_FIELD, 0, 0, 0, 0);
        return 0;
    }
    while (current_lba < end_lba) {
        sxs_u64 lpn = current_lba / metadata->sectors_per_page;
        sxs_u32 page_sector = current_lba % metadata->sectors_per_page;
        sxs_u32 sectors = metadata->sectors_per_page - page_sector;
        sxs_u64 ppa;
        sxs_u64 latency = 0;
        sxs_u64 host_offset;
        sxs_u64 bytes;

        if (sectors > end_lba - current_lba) {
            sectors = end_lba - current_lba;
        }
        if (lpn >= metadata->total_logical_pages ||
            block_state_read_u64(context, BLOCK_MAP_OBJECT, lpn, &ppa) != 0) {
            sxs_completion_status_set(BLOCK_NVME_INVALID_FIELD, 0, 0, 0, 0);
            return 0;
        }
        block_bytes_zero(context->scratch + BLOCK_PAGE_OFFSET,
                         metadata->page_size);
        if (ppa != BLOCK_UNMAPPED &&
            (sxs_s64)sxs_ppa_validate(ppa, 0, 0, 0, 0) == 1 &&
            block_page_read(context, ppa, metadata->page_size,
                            &latency) != 0) {
            sxs_completion_status_set(BLOCK_NVME_INTERNAL_ERROR, 0, 0, 0, 0);
            return 0;
        }
        if (ppa != BLOCK_UNMAPPED &&
            BLOCK_ON_PAGE_READ(context, lpn, ppa) != 0) {
            sxs_completion_status_set(BLOCK_NVME_INTERNAL_ERROR, 0, 0, 0, 0);
            return 0;
        }
        host_offset = (current_lba - context->event.nvme.lba) *
                      metadata->sector_size;
        bytes = (sxs_u64)sectors * metadata->sector_size;
        if (sxs_request_write(host_offset,
                              BLOCK_PAGE_OFFSET +
                                  page_sector * metadata->sector_size,
                              bytes, 0, 0) != 0) {
            sxs_completion_status_set(BLOCK_NVME_INTERNAL_ERROR, 0, 0, 0, 0);
            return 0;
        }
        if (latency > maximum_latency) {
            maximum_latency = latency;
        }
        current_lba += sectors;
    }
    sxs_completion_status_set(BLOCK_NVME_SUCCESS, 0, 0, 0, 0);
    return maximum_latency;
}

static sxs_u64 block_write_action(struct sxs_bpf_context *context,
                                  struct block_bpf_metadata *metadata)
{
    sxs_u64 maximum_latency = 0;
    sxs_u64 end_lba = context->event.nvme.lba +
                      context->event.nvme.nsecs;
    sxs_u64 current_lba = context->event.nvme.lba;

    if (end_lba < current_lba) {
        sxs_completion_status_set(BLOCK_NVME_INVALID_FIELD, 0, 0, 0, 0);
        return 0;
    }
    if (metadata->free_eswds <= metadata->gc_urgent_watermark) {
        block_gc(context, metadata, 1);
    }
    while (current_lba < end_lba) {
        sxs_u64 lpn = current_lba / metadata->sectors_per_page;
        sxs_u32 page_sector = current_lba % metadata->sectors_per_page;
        sxs_u32 sectors = metadata->sectors_per_page - page_sector;
        sxs_u64 old_ppa;
        sxs_u64 new_ppa;
        sxs_u64 latency = 0;
        sxs_u64 ignored_read_latency = 0;
        sxs_u64 host_offset;
        sxs_u64 bytes;

        if (sectors > end_lba - current_lba) {
            sectors = end_lba - current_lba;
        }
        if (lpn >= metadata->total_logical_pages ||
            block_state_read_u64(context, BLOCK_MAP_OBJECT,
                                 lpn, &old_ppa) != 0 ||
            block_rotate_if_full(context, metadata) != 0) {
            sxs_completion_status_set(BLOCK_NVME_INVALID_FIELD, 0, 0, 0, 0);
            return 0;
        }
        block_bytes_zero(context->scratch + BLOCK_PAGE_OFFSET,
                         metadata->page_size);
        if (old_ppa != BLOCK_UNMAPPED &&
            (sxs_s64)sxs_ppa_validate(old_ppa, 0, 0, 0, 0) == 1 &&
            block_page_read(context, old_ppa, metadata->page_size,
                            &ignored_read_latency) != 0) {
            sxs_completion_status_set(BLOCK_NVME_INTERNAL_ERROR, 0, 0, 0, 0);
            return 0;
        }
        host_offset = (current_lba - context->event.nvme.lba) *
                      metadata->sector_size;
        bytes = (sxs_u64)sectors * metadata->sector_size;
        if (sxs_request_read(host_offset,
                             BLOCK_PAGE_OFFSET +
                                 page_sector * metadata->sector_size,
                             bytes, 0, 0) != 0 ||
            block_page_append(context, lpn, old_ppa,
                              metadata->current_eswd,
                              metadata->page_size, &new_ppa,
                              &latency) != 0) {
            sxs_completion_status_set(BLOCK_NVME_INTERNAL_ERROR, 0, 0, 0, 0);
            return 0;
        }
        if (old_ppa != BLOCK_UNMAPPED) {
            if (BLOCK_HANDLE_OLD_PAGE(context, lpn, old_ppa) != 0) {
                sxs_completion_status_set(BLOCK_NVME_INTERNAL_ERROR,
                                          0, 0, 0, 0);
                return 0;
            }
        }
        if (block_update_mapping(context, lpn, old_ppa, new_ppa) != 0) {
            sxs_completion_status_set(BLOCK_NVME_INTERNAL_ERROR, 0, 0, 0, 0);
            return 0;
        }
        if (BLOCK_AFTER_NEW_PAGE(context, lpn, old_ppa, new_ppa) != 0) {
            sxs_completion_status_set(BLOCK_NVME_INTERNAL_ERROR, 0, 0, 0, 0);
            return 0;
        }
        if (latency > maximum_latency) {
            maximum_latency = latency;
        }
        current_lba += sectors;
    }
    if (block_rotate_if_full(context, metadata) != 0 &&
        metadata->free_eswds == 0) {
        /* The data is committed; background GC may recover space next. */
    }
    sxs_completion_status_set(BLOCK_NVME_SUCCESS, 0, 0, 0, 0);
    return maximum_latency;
}

static sxs_u64 block_dsm_action(struct sxs_bpf_context *context,
                                struct block_bpf_metadata *metadata)
{
    struct sxs_bpf_dsm_range *range =
        (struct sxs_bpf_dsm_range *)(context->scratch + BLOCK_DSM_OFFSET);
    sxs_u64 total_pages = 0;

    /* Validate the complete request before applying any irreversible trim. */
    for (sxs_u32 range_index = 0; range_index < 256; range_index++) {
        sxs_s64 rc = sxs_dsm_range_get(range_index, BLOCK_DSM_OFFSET,
                                       0, 0, 0);
        sxs_u64 end_lba;
        sxs_u64 first_lpn;
        sxs_u64 last_lpn;
        sxs_u64 pages;

        if (rc == -SXS_BPF_ENOENT) {
            break;
        }
        if (rc != 0) {
            sxs_completion_status_set(BLOCK_NVME_INTERNAL_ERROR,
                                      0, 0, 0, 0);
            return 0;
        }
        if (range->lba_count == 0) {
            continue;
        }
        end_lba = range->start_lba + range->lba_count - 1;
        if (end_lba < range->start_lba) {
            sxs_completion_status_set(BLOCK_NVME_INVALID_FIELD,
                                      0, 0, 0, 0);
            return 0;
        }
        first_lpn = range->start_lba / metadata->sectors_per_page;
        last_lpn = end_lba / metadata->sectors_per_page;
        pages = last_lpn - first_lpn + 1;
        if (first_lpn >= metadata->total_logical_pages ||
            last_lpn >= metadata->total_logical_pages ||
            pages > BLOCK_MAX_DSM_PAGES - total_pages) {
            sxs_completion_status_set(BLOCK_NVME_INVALID_FIELD,
                                      0, 0, 0, 0);
            return 0;
        }
        total_pages += pages;
    }

    for (sxs_u32 range_index = 0; range_index < 256; range_index++) {
        sxs_s64 rc = sxs_dsm_range_get(range_index, BLOCK_DSM_OFFSET,
                                       0, 0, 0);
        sxs_u64 end_lba;
        sxs_u64 start_lpn;
        sxs_u64 end_lpn;

        if (rc == -SXS_BPF_ENOENT) {
            break;
        }
        if (rc != 0 || range->lba_count == 0) {
            continue;
        }
        end_lba = range->start_lba + range->lba_count - 1;
        if (end_lba < range->start_lba) {
            continue;
        }
        start_lpn = range->start_lba / metadata->sectors_per_page;
        end_lpn = end_lba / metadata->sectors_per_page;
        for (sxs_u64 lpn = start_lpn; lpn <= end_lpn; lpn++) {
            sxs_u64 ppa;

            if (lpn >= metadata->total_logical_pages ||
                block_state_read_u64(context, BLOCK_MAP_OBJECT,
                                     lpn, &ppa) != 0 ||
                ppa == BLOCK_UNMAPPED) {
                continue;
            }
            if (BLOCK_HANDLE_OLD_PAGE(context, lpn, ppa) != 0) {
                return SXS_BPF_ACTION_ERROR;
            }
            if (block_update_mapping(context, lpn, ppa,
                                     BLOCK_UNMAPPED) != 0) {
                return SXS_BPF_ACTION_ERROR;
            }
        }
    }
    sxs_completion_status_set(BLOCK_NVME_SUCCESS, 0, 0, 0, 0);
    return 0;
}

static sxs_u64 block_policy_init(struct sxs_bpf_context *context)
{
    struct sxs_bpf_geometry *geometry =
        (struct sxs_bpf_geometry *)(context->scratch + BLOCK_GEOMETRY_OFFSET);
    struct block_bpf_metadata *metadata =
        (struct block_bpf_metadata *)(context->scratch +
                                       BLOCK_METADATA_OFFSET);
    struct sxs_bpf_eswd_config *eswd_config =
        (struct sxs_bpf_eswd_config *)(context->scratch +
                                        BLOCK_PAGE_REQUEST_OFFSET);
    struct sxs_bpf_namespace_config *namespace_config =
        (struct sxs_bpf_namespace_config *)(context->scratch +
                                             BLOCK_PAGE_RESULT_OFFSET);
    sxs_u64 total_eswds;
    sxs_u64 page_size;

    if (sxs_geometry_get(BLOCK_GEOMETRY_OFFSET, 0, 0, 0, 0) != 0 ||
        geometry->total_luns == 0) {
        return SXS_BPF_ACTION_ERROR;
    }
    total_eswds = geometry->total_blocks_log / geometry->total_luns;
    page_size = (sxs_u64)geometry->sectors_per_page * geometry->sector_size;
    if (total_eswds < 2 || total_eswds > 0xffffffffULL ||
        page_size == 0 || page_size > SXS_BPF_MAX_PAGE_BYTES) {
        return SXS_BPF_ACTION_ERROR;
    }
    if (sxs_state_create(BLOCK_MAP_OBJECT, sizeof(sxs_u64),
                         geometry->total_pages_log,
                         SXS_BPF_STATE_INIT_U64, BLOCK_UNMAPPED) != 0 ||
        sxs_state_create(BLOCK_REVERSE_MAP_OBJECT, sizeof(sxs_u64),
                         geometry->total_pages_log,
                         SXS_BPF_STATE_INIT_U64, BLOCK_UNMAPPED) != 0 ||
        sxs_state_create(BLOCK_METADATA_OBJECT, sizeof(*metadata), 1,
                         0, 0) != 0 ||
        sxs_state_create(BLOCK_ESWD_CLASS_OBJECT, 1, total_eswds,
                         0, 0) != 0) {
        return SXS_BPF_ACTION_ERROR;
    }

    if (!(context->flags & SXS_BPF_FLAG_STATE_RESTORED)) {
        *metadata = (struct block_bpf_metadata) {
            .total_logical_pages = geometry->total_pages_log,
            .total_eswds = total_eswds,
            .pages_per_eswd = geometry->total_luns *
                              geometry->pages_per_block,
            .sectors_per_page = geometry->sectors_per_page,
            .sector_size = geometry->sector_size,
            .page_size = page_size,
            .current_eswd = 0,
            .free_eswds = total_eswds - 1,
            .gc_low_watermark = total_eswds / 4 ? total_eswds / 4 : 1,
            .gc_urgent_watermark = total_eswds / 20 ?
                                   total_eswds / 20 : 1,
        };
        if (block_write_metadata() != 0 ||
            block_state_write_class(context, 0,
                                    BLOCK_ESWD_CURRENT) != 0) {
            return SXS_BPF_ACTION_ERROR;
        }
    }

    eswd_config->striping_level = 0;
    eswd_config->blocks_per_eswd = geometry->total_luns;
    namespace_config->csi = 0;
    namespace_config->noiob = 0;
    namespace_config->nsze = geometry->total_pages_log *
                             geometry->sectors_per_page;
    namespace_config->ncap = namespace_config->nsze;
    namespace_config->nuse = namespace_config->ncap;
    namespace_config->namespace_blob_length = 0;
    namespace_config->controller_blob_length = 0;
    if (sxs_eswd_config_stage(BLOCK_PAGE_REQUEST_OFFSET, 0, 0, 0, 0) != 0 ||
        sxs_namespace_config_stage(BLOCK_PAGE_RESULT_OFFSET, 0, 0, 0, 0) != 0 ||
        sxs_ftl_finalize_stage(0, 0, 0, 0, 0) != 0 ||
        sxs_subscribe(SXS_BPF_EVENT_NVME_IO, BLOCK_NVME_READ,
                      BLOCK_PAIR_READ, 0, 0) != 0 ||
        sxs_subscribe(SXS_BPF_EVENT_NVME_IO, BLOCK_NVME_WRITE,
                      BLOCK_PAIR_WRITE, 0, 0) != 0 ||
        sxs_subscribe(SXS_BPF_EVENT_NVME_IO, BLOCK_NVME_DSM,
                      BLOCK_PAIR_DSM, 0, 0) != 0 ||
        sxs_subscribe(SXS_BPF_EVENT_BACKGROUND, 0,
                      BLOCK_PAIR_BACKGROUND_GC, 0, 0) != 0 ||
        BLOCK_EXTRA_INIT(context, geometry, metadata) != 0) {
        return SXS_BPF_ACTION_ERROR;
    }
    return 0;
}

static sxs_u64 block_policy_condition(struct sxs_bpf_context *context)
{
    struct block_bpf_metadata *metadata;

    switch (context->pair_id) {
    case BLOCK_PAIR_READ:
        return context->event.nvme.opcode == BLOCK_NVME_READ;
    case BLOCK_PAIR_WRITE:
        return context->event.nvme.opcode == BLOCK_NVME_WRITE;
    case BLOCK_PAIR_DSM:
        return context->event.nvme.opcode == BLOCK_NVME_DSM;
    case BLOCK_PAIR_BACKGROUND_GC:
        if (block_read_metadata(context, &metadata) != 0) {
            return 0;
        }
        return metadata->free_eswds <= metadata->gc_low_watermark;
    default:
        return BLOCK_EXTRA_CONDITION(context);
    }
}

static sxs_u64 block_policy_action(struct sxs_bpf_context *context)
{
    struct block_bpf_metadata *metadata;

    if (block_read_metadata(context, &metadata) != 0) {
        return SXS_BPF_ACTION_ERROR;
    }
    switch (context->pair_id) {
    case BLOCK_PAIR_READ:
        return block_read_action(context, metadata);
    case BLOCK_PAIR_WRITE:
        return block_write_action(context, metadata);
    case BLOCK_PAIR_DSM:
        return block_dsm_action(context, metadata);
    case BLOCK_PAIR_BACKGROUND_GC:
        block_gc(context, metadata, 0);
        return 0;
    default:
        return BLOCK_EXTRA_ACTION(context);
    }
}

static sxs_u64 block_policy_dispatch(void *memory, sxs_u64 memory_size)
{
    struct sxs_bpf_context *context = memory;

    if (!context || memory_size != sizeof(*context) ||
        context->abi_version != SXS_BPF_ABI_VERSION ||
        context->context_size != sizeof(*context)) {
        return SXS_BPF_ACTION_ERROR;
    }
    switch (context->phase) {
    case SXS_BPF_PHASE_INIT:
        return block_policy_init(context);
    case SXS_BPF_PHASE_CONDITION:
        return block_policy_condition(context);
    case SXS_BPF_PHASE_ACTION:
        return block_policy_action(context);
    default:
        return SXS_BPF_ACTION_ERROR;
    }
}

#endif /* SXS_BLOCK_POLICY_BPF_CORE_H */
