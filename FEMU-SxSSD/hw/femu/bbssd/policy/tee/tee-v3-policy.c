...
{
 struct tee_v3_policy_context*c=opaque;struct tee_v1_bitmap bitmap={0};struct tee_v2_passive_metadata*records;uint32_t i;int rc=-1;
 ...
 records=malloc(sizeof(*records)*(cache->passive_count+1));if(!records)goto out;for(i=0;i<cache->passive_count;i++)records[i]=cache->passive_records[i];records[cache->passive_count]=*passive;
 rc=tee_v3_storage_persist(c->storage,&bitmap,records,cache->passive_count+1);free(records);if(rc!=0)tee_v3_pending_record_error(&c->pending,rc,0);
