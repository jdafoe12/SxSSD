...
int raw_flash_init(struct RawFlash *fb, SsdDramBackend *mbe, const BbCtrlParams *bbp)
{
    ...
    fb->migration_page_buf_size =
        (size_t)spp->secs_per_pg * (size_t)spp->secsz;
...
static uint64_t raw_flash_advance_status(struct RawFlash *fb,
                                         const struct ppa *ppa,
                                         struct RawFlashEvent *event)
{
    ...
    switch (event ? event->cmd : RAW_FLASH_EVENT_READ) {
    case RAW_FLASH_EVENT_READ:
...
{
    event->cmd = RAW_FLASH_EVENT_READ;
    event->count = count;
...
static void fill_write_event(struct RawFlash *fb, struct RawFlashEvent *event, struct ppa *ppa_list, uint64_t count, int lat)
{
    event->cmd = RAW_FLASH_EVENT_WRITE;
    event->count = count;
...
static void fill_erase_event(struct RawFlash *fb, struct RawFlashEvent *event, struct pba *pba, uint64_t count, int lat)
{
    const struct ssdparams *spp = &fb->sp;
    event->cmd = RAW_FLASH_EVENT_ERASE;
    event->count = count;
    /* Ownership: upper layer allocates/frees status_list. Backend only fills it. */
    for (uint64_t i = 0; i < count; ++i) {
...
void raw_flash_mark_page_valid(struct RawFlash *fb, const struct ppa *ppa)
{
 ...
 //       printf("[DEBUG] mark_page_valid: physical block 2048 page %lu (ch=%d, lun=%d, pl=%d, blk=%d, pg=%d), current state=%d\n",
  ...
  //  }
   ...
   //     printf("raw_flash_mark_page_valid: page %" PRIu64 " (physical blk %" PRIu64 ") is in state %d (ch=%d, lun=%d, pl=%d, blk=%d, pg=%d)\n",
...
static int raw_flash_latency(struct RawFlash *fb, struct ppa *ppa_list,
                               uint64_t ppa_count, struct RawFlashEvent *event)
{
    ...
    int max_lat = 0;
    for (uint64_t i = 0; i < ppa_count; ++i) {
        int lat = (int)raw_flash_advance_status(fb, &ppa_list[i], event);
        if (lat > max_lat) {
            max_lat = lat;
        }
    }
...
int raw_flash_read(struct RawFlash *fb, uint8_t *buffer, struct ppa *ppa_list,
                         ...
                         struct RawFlashEvent *event)
{
    ...
    int lat = raw_flash_latency(fb, ppa_list, ppa_count, event);
    if (event) {
                          ...
                          uint64_t block_count, struct RawFlashEvent *event)
{
    ...
    for (uint64_t i = 0; i < block_count; ++i) {
        ...
        int lat = (int)raw_flash_advance_status(fb, &ppa, event);
        if (lat > max_lat) {
            max_lat = lat;
        }
    }
...
/* OOB management */
static int find_oob_range(const struct RawFlash *fb, const bool *occupied,
...
static bool *build_oob_occupancy(const struct RawFlash *fb)
{
    bool *occupied;
    int i;
