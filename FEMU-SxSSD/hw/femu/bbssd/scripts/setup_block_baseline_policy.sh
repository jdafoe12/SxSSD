...
POLICY_PATH="${POLICY_PATH:-$POLICY_DIR/block-interface-policy-baseline.bpf.o}"

fail() {
    echo "[setup-block-baseline] ERROR: $*" >&2
    exit 1
}
...
echo "[setup-block-baseline] Building policy image and policyctl"
make -C "$POLICY_DIR"
make -C "$SCRIPT_DIR"
