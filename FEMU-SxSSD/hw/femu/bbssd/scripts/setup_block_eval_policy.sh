...
POLICY_ID="${POLICY_ID:-100}"
POLICY_VERSION="${POLICY_VERSION:-1}"
SESSION_MODE="${SESSION_MODE:-confidential}"
POLICY_PATH="${POLICY_PATH:-$POLICY_DIR/block-interface-policy-eval.bpf.o}"

fail() {
    echo "[setup-block-eval] ERROR: $*" >&2
    exit 1
}
...
echo "[setup-block-eval] Building policy image and policyctl"
make -C "$POLICY_DIR"
make -C "$SCRIPT_DIR"
