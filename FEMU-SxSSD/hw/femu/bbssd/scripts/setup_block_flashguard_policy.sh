...
POLICY_ID="${POLICY_ID:-102}"
POLICY_VERSION="${POLICY_VERSION:-1}"
SESSION_MODE="${SESSION_MODE:-confidential}"
POLICY_PATH="${POLICY_PATH:-$POLICY_DIR/block-interface-policy-flashguard.bpf.o}"

fail() {
    echo "[setup-block-flashguard] ERROR: $*" >&2
    exit 1
}
...
echo "[setup-block-flashguard] Building policy image and policyctl"
make -C "$POLICY_DIR"
make -C "$SCRIPT_DIR"
...
echo "[setup-block-flashguard] Opening session on $CTRL_DEVICE"
"$SCRIPT_DIR/policyctl" --mode "$SESSION_MODE" session "$CTRL_DEVICE"

echo "[setup-block-flashguard] Installing block-flashguard policy"
"$SCRIPT_DIR/policyctl" install "$CTRL_DEVICE" "$POLICY_PATH" "$POLICY_ID" "$POLICY_VERSION"

echo "[setup-block-flashguard] Activating block-flashguard policy"
"$SCRIPT_DIR/policyctl" activate "$CTRL_DEVICE" "$POLICY_ID"

echo "[setup-block-flashguard] Rescanning namespaces"
nvme ns-rescan "$CTRL_DEVICE"
sleep 1

[ -b "$NS_DEVICE" ] || fail "Namespace device did not appear: $NS_DEVICE"

echo "[setup-block-flashguard] Ready: $NS_DEVICE"
