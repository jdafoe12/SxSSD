...
POLICY_PATH="${POLICY_PATH:-$POLICY_DIR/block-interface-policy.so}"

fail() {
    echo "[setup-block] ERROR: $*" >&2
    exit 1
}
...
echo "[setup-block] Building policy image and policyctl"
make -C "$POLICY_DIR"
make -C "$SCRIPT_DIR"
