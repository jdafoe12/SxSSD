#include "block-interface-policy-buffered.h"
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

static struct buffered_block_policy_context *g_ctx = NULL;

static inline struct buffered_block_policy_context *get_policy_ctx(struct ssd *ssd)
{
    (void)ssd;
    return g_ctx;
}

static PseudoPpa get_maptbl_ent(struct buffered_block_policy_context *ctx, uint64_t lpn)
{
...
static void set_rmap_ent(struct buffered_block_policy_context *ctx, uint64_t lpn, PseudoPpa *ppa)
{
...
static uint64_t get_rmap_ent(struct buffered_block_policy_context *ctx, PseudoPpa *ppa)
{
    uint64_t pgidx = ctx->api->ppa_to_pgidx(ctx->ssd, ppa);
    if (pgidx >= ctx->tt_pgs_log) {
        return INVALID_LPN;
...
static void set_maptbl_ent(struct buffered_block_policy_context *ctx, uint64_t lpn, PseudoPpa *ppa)
{
    uint64_t old_pgidx;
    uint64_t pgidx;
...
static bool valid_lpn(struct buffered_block_policy_context *ctx, uint64_t lpn)
{
    return lpn < ctx->tt_pgs_log;
}

static void update_eswd_after_invalidate(struct buffered_block_policy_context *ctx, PseudoPpa *ppa)
{
...
static void mark_page_invalid(struct buffered_block_policy_context *ctx, PseudoPpa *ppa)
{
    ctx->api->mark_page_invalid(ctx->ssd, ppa);
    update_eswd_after_invalidate(ctx, ppa);
}

static struct eswd *get_next_free_eswd(struct buffered_block_policy_context *ctx)
{
    struct buffered_eswd_free_node *node = QTAILQ_FIRST(&ctx->free_list);
...
static int switch_to_next_eswd(struct buffered_block_policy_context *ctx)
{
...
static void rotate_if_full(struct buffered_block_policy_context *ctx)
{
...
static bool should_gc(struct buffered_block_policy_context *ctx)
{
    return ctx->free_cnt <= ctx->api->get_gc_thres_lines(ctx->ssd);
}

static bool should_gc_high(struct buffered_block_policy_context *ctx)
{
    return ctx->free_cnt <= ctx->api->get_gc_thres_lines_high(ctx->ssd);
}

static struct eswd *select_victim_eswd(struct buffered_block_policy_context *ctx, bool force)
{
    struct buffered_eswd_victim_node *node;
    struct eswd *victim;
    uint32_t pgs_per_eswd = ctx->api->get_eswd_layout(ctx->ssd)->pgs_per_eswd;

    node = (struct buffered_eswd_victim_node *)pqueue_peek(ctx->victim_pq);
    if (!node) {
        return NULL;
    }
    victim = node->eswd;
...
{
    struct buffered_block_policy_context *ctx = ctx_ptr;
...
{
    struct buffered_block_policy_context *ctx = context;
    (void)src_eswd_id;
    (void)page_index;
...
static int do_gc(struct buffered_block_policy_context *ctx, bool force)
{
...
{
    struct buffered_block_policy_context *ctx = opaque;
...
static uint64_t buffered_block_write(struct ssd *ssd, struct NvmeCommandEvent *event)
{
    struct buffered_block_policy_context *ctx = get_policy_ctx(ssd);
    uint8_t *req_buf;
    uint64_t req_size = 0;
    uint64_t lat = 0;
    ...
    for (uint64_t lpn = event->start_lpn; lpn <= event->end_lpn; lpn++) {
        ...
        lat += ctx->api->commit_page_to_eswd(ctx->ssd, ctx->cur_eswd_id, page_buf,
...
static uint64_t buffered_block_trim(struct ssd *ssd, struct NvmeCommandEvent *event)
{
    struct buffered_block_policy_context *ctx = get_policy_ctx(ssd);
                               ...
                               struct FtlPolicyAPI *api, void *context)
{
    (void)api;
    (void)context;
    return buffered_block_write(ssd, event);
}
                             ...
                             struct FtlPolicyAPI *api, void *context)
{
    (void)api;
    (void)context;
    return buffered_block_trim(ssd, event);
}
...
{
    return (pqueue_pri_t)((struct buffered_eswd_victim_node *)a)->eswd->vpc;
}

static inline void victim_set_pri(void *a, pqueue_pri_t pri)
{
    ((struct buffered_eswd_victim_node *)a)->eswd->vpc = (int)pri;
}

static inline size_t victim_get_pos(void *a)
{
    return ((struct buffered_eswd_victim_node *)a)->pos;
}

static inline void victim_set_pos(void *a, size_t pos)
{
    ((struct buffered_eswd_victim_node *)a)->pos = pos;
}

static void gc_update_mapping(struct ssd *ssd, PseudoPpa *old_ppa, PseudoPpa *new_ppa)
{
    struct buffered_block_policy_context *ctx = get_policy_ctx(ssd);
    uint64_t lpn;
...
{
    struct buffered_block_policy_context *ctx;
    ...
    g_ctx = ctx;
    api->gc_update_mapping = gc_update_mapping;
